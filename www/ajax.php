...
if(isset($_POST['sign_up'])) {
    ...
    $insert = "INSERT INTO user (name,username,email,password,type) VALUES ('$name','$user_name','$email_id','$password', '$user_type')";
    ...
    $insert_result = mysqli_query($connection,$insert);
        if ($insert_result) {
            ...
            $response['done'] = false;
            $response['data'] = "DataBase Error in registration";
        }
    echo json_encode($response);
}

if (isset($_POST['login'])) {
    // Add SQL injection prevention before making this query
    $email = $_POST['email'];
    $password = $_POST['password'];
        ...
        if (mysqli_num_rows($result) == 1) {
            ...
            switch ($user['type']) {
                case 1:
                    ...
                    break;
                case 2:
                    header('Location:index.php?emp_home');
                    break;
                case 3:
...
if (isset($_POST['add_room'])) {
    ...
    if ($room_no != '') {
        ...
        if (mysqli_num_rows(mysqli_query($connection, $sql)) >= 1) {
            ...
            $query = "INSERT INTO room (room_type_id,room_no,check_in_status,check_out_status,deleteStatus) VALUES ('$room_type_id','$room_no',0,0,0);";
            $result = mysqli_query($connection, $query);
...
if (isset($_POST['add_employee'])) {
    ...
    if ($staff_type == '' && $shift == '' && $salary == ''){
        $response['done'] = false;
        $response['data'] = "Please Enter Details";
    }else{
        $customer_sql = "INSERT INTO staff (emp_name,emp_username,staff_type_id,shift_id,id_card_type,id_card_no,address,contact_no,salary) VALUES ('$name','','$staff_type','$shift','$id_card_id','$id_card_no','$address','$contact_no','$salary')";
        $customer_result = mysqli_query($connection, $customer_sql);
        ...
        if ($customer_result && $insert_result) {
            $response['done'] = true;
            $response['data'] = 'Successfully Hired';
        } else {
            $response['done'] = false;
            $response['data'] = "DataBase Error in status change";
        }
